...
COLL_COEF="${URBANZERO_COLLISION_COEF:-}"
ACTIVATE="source '$VENV' && export PYTHONPATH=\$PYTHONPATH:'$CARLA_PYTHONAPI' && export URBANZERO_SEED='$SEED' && export URBANZERO_BC_WEIGHTS='$BC_WEIGHTS' && export URBANZERO_IDLE_COST_COEF='$IDLE_COEF' && export URBANZERO_REALLY_STUCK_STEPS='$STUCK_STEPS' && export URBANZERO_COLLISION_COEF='$COLL_COEF' && cd '$REPO'"
